...
select if(round(count(distinct a.player_id)/count(distinct b.player_id),2) is null,0,round(count(distinct a.player_id)/count(distinct b.player_id),2)) as fraction from cl as a cross join Activity as b